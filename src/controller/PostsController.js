...
const PostsController = {
    ...
    getByUser: (req,res,next)=>{
        ...
        .sort({timestamps:-1}).exec((err,result)=>{
            if (err) throw err
    ...
    getByMentions: (req,res,next)=>{
        ...
        .populate('author').sort({timestamps:-1}).exec((err,result)=>{
            if (err) throw err
            if (!result) return res.status(404).send('empty post')
    ...
    post: (req,res,next)=>{
        ...
        post.save((err,result)=>{
            ...
            // return res.json(result)
            if(tag){
            Posts.findByIdAndUpdate(result._id,
                {$push:{tag:{$each:tag}}},{new:true}
                ).exec((err,data)=>{
                    if(err) throw err
                    
                    let notif_message = 'just tags you in their post'
                    for (var i = 0; i < tag.length; i++) {
                    User.findOneAndUpdate({username:tag[i]},
                        {$push:{notification:{
                                subject:username,
                                refer:result._id,
                                notif_type:'tag',
        ...
        Posts.findByIdAndUpdate(
            ...
            {new:true}
                ...
                User.findOne(
                    ...
                    notification:{subject:username,refer:postId,notif_type:'likes'}})
                    .exec((err,notif)=>{
                    ...
                    if(username!==result.username&&alreadyNotif==null){
                        User.findOneAndUpdate(
                            ...
                            }},{new:true}).exec(()=>{console.log('notif added')})
                    }
                    })
                return res.json(result) 
            })
